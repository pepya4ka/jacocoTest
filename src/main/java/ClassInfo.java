...
//        }
        File file = new File("src/test/java/calc/CalculatorTest.java");
        analyzer.analyzeAll(file);
    }

    public void visitCoverage(final IClassCoverage coverage) {
        out.printf("class name:   %s%n",    coverage.getName());
        out.printf("class id:     %016x%n", coverage.getId());
        out.printf("instructions: %s%n",    coverage.getInstructionCounter().getTotalCount());
        out.printf("branches:     %s%n",    coverage.getBranchCounter().getTotalCount());
        out.printf("lines:        %s%n",    coverage.getLineCounter().getTotalCount());
        out.printf("methods:      %s%n",    coverage.getMethodCounter().getTotalCount());
        out.printf("complexity:   %s%n%n",  coverage.getComplexityCounter().getTotalCount());
    }
...
//        new ClassInfo(System.out).execute(args);
        new ClassInfo(System.out).execute();
    }
}